...
Changes vs original:
  ...
  - Flash Attention 2: faster attention for long prompts
  - TF32 matmuls + cuDNN benchmark: free speedup on Ampere+ GPUs
  - num_generations default 8 → 6
  - epochs default 3 → 1
...
def train(args: argparse.Namespace) -> None:
    # Free ~10-20% on Ampere+ GPUs via TF32 tensor cores; no accuracy cost
    torch.set_float32_matmul_precision("high")
    # Let cuDNN profile and pick the fastest kernel for our fixed input shapes
    ...
    print([n for n, _ in model.named_modules() if "proj" in n or "attn" in n or "fc" in n][:20])
    ...
    # Attention + one FFN layer only (same as lab's sft.py).
    # Fewer hooks per forward pass than "all-linear"; check layer names with
    # `print([n for n, _ in model.named_modules()])` if PEFT raises a warning.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RLVR pipeline for PIT stock return prediction (speed-optimized)")
    ...
    parser.add_argument("--num_generations", type=int, default=6,
                        help="Completions per prompt for group-relative advantage (was 8)")

    # Training
    parser.add_argument("--epochs", type=int, default=1,
                        help="Training epochs (was 3; RLVR typically needs fewer than SFT)")
