"""
RLVR fine-tuning pipeline v3 — Gaussian reward.

The model now predicts a continuous return value in % (e.g. +2.3 or -1.5)
instead of a binary +1/-1.  The reward is a Gaussian centred on the true return:

    reward = exp(-((pred_pct/100 - true_ret)² / (2 × sigma²)))

reward ∈ (0, 1].  Perfect prediction → 1.  Wrong direction → near 0.
...
def extract_prediction(text: str) -> float | None:
    """Extract the last percentage value from the completion.

    Tries numbers followed by % first, then falls back to signed numbers.
    Returns the value in percentage points (e.g. 2.3 for +2.3%).
    """
    # Primary: number explicitly followed by %
    ...
    if matches:
        try:
            ...
            pass
    # Fallback: signed number (avoids matching bare integers like years)
...
_SIGMA = 0.02  # overridden in train() from args.reward_sigma


def gaussian_reward(completions: list[str], ret: list[float], **_) -> list[float]:
    """
    Gaussian reward centred on the true return.

    reward = exp(-((pred_decimal - true_ret)² / (2 × _SIGMA²)))

    pred is parsed in percentage points and converted to decimal before comparison
    with true_ret (which is already a decimal, e.g. 0.02 for +2%).
    ...
    for completion, r in zip(completions, ret):
        pred_pct = extract_prediction(completion)
        if pred_pct is None:
            rewards.append(0.0)
        else:
            pred_decimal = pred_pct / 100.0
            rewards.append(math.exp(-((pred_decimal - r) ** 2) / (2 * _SIGMA ** 2)))
    return rewards
...
class RewardLogger(TrainerCallback):
    ...
    def on_log(self, _args, state: TrainerState, _control: TrainerControl, logs=None, **_kwargs):
        ...
        print(
            ...
            f"gaussian_reward={reward:.4f} ± {reward_std:.4f}  "
...
def train(args: argparse.Namespace) -> None:
    ...
    if master_process and use_wandb:
        wandb.init(
            ...
            tags=["rlvr", "grpo", "ddp", "gaussian-reward", args.model_name.split("/")[-1]],
        )
...
def parse_args() -> argparse.Namespace:
    ...
    # Reward
    parser.add_argument("--reward_sigma", type=float, default=0.02,
                        help="Gaussian sigma in decimal (0.02 = 2%%). Controls tolerance around true return.")
